"""
def load_policy(logdir): # This should be giving us policy
...
def load_env(label, headless=False): # This should be giving us environments
    ...
    # TODO : check is the envs should be altered
    ...
    # TODO : check is the below things needs to be altered
...
def train_go1(headless=True):
    ...
    from go1_gym.navigation.navigation_robot_config import Nfg
    from go1_gym.navigation.navigation_robot import NavRobot

    config_go1(Cfg)

    # Cfg.commands.num_lin_vel_bins = 30
    Nfg.commands.num_lin_pos_bins = 30 # ***
    # Cfg.commands.num_ang_vel_bins = 30

    # Cfg.curriculum_thresholds.tracking_ang_vel = 0.7
    Nfg.curriculum_thresholds.tracking_yaw = 0.6
    # Cfg.curriculum_thresholds.tracking_lin_vel = 0.8
    Nfg.curriculum_thresholds.tracking_lin_pos = 0.8
    ...
    Nfg.commands.distributional_commands = True
    # Cfg.domain_rand.lag_timesteps = 6
    Nfg.domain_rand.lag_timesteps = 6
    # Cfg.domain_rand.randomize_lag_timesteps = True
    Nfg.domain_rand.randomize_lag_timesteps = True
    # Cfg.control.control_type = "actuator_net"
    ...
    # TODO : do we need this below ? FALSE though 
    ...
    Nfg.env.num_privileged_obs = 0
    Cfg.env.num_observation_history = 30
    # Cfg.reward_scales.feet_contact_forces = 0.0

    # Cfg.domain_rand.rand_interval_s = 4
    # Cfg.commands.num_commands = 15
    Nfg.commands.num_commands = 3
    # Cfg.env.observe_two_prev_actions = True
    Nfg.env.observe_two_prev_actions = True
    # Cfg.env.observe_yaw = False
    Nfg.env.observe_yaw = False
    # Cfg.env.num_observations = 70
    Nfg.env.num_observations = 70
    # Cfg.env.num_scalar_observations = 70
    Nfg.env.num_scalar_observations = 70
    # TODO : 
    # Cfg.env.observe_gait_commands = True # _step_contact_targets -> LCMAgent which is in deploy 
    # Cfg.env.observe_timing_parameter = False # sets some gaits
    # Cfg.env.observe_clock_inputs = True
    Nfg.env.observe_clock_inputs = True
    ...
    # Cfg.terrain.border_size = 0.0   # related to height fields and trimesh creation
    ...
    Nfg.commands.resampling_time = 10# necessary for post physics step callback and resample commands 

    # TODO : check the rewards once again 
    # Cfg.reward_scales.feet_slip = 0.0 #-0.04
    # Cfg.reward_scales.action_smoothness_1 = 0.0 #-0.1
    # Cfg.reward_scales.action_smoothness_2 = 0.0 #-0.1
    # Cfg.reward_scales.dof_vel = 0.0 #-1e-4
    # Cfg.reward_scales.dof_pos = 0.0 #-0.0
    # Cfg.reward_scales.jump = 0.0 #10.0
    # Cfg.reward_scales.base_height = 0.0
    # Cfg.rewards.base_height_target = 0.0 #0.30
    # Cfg.reward_scales.estimation_bonus = 0.0
    # Cfg.reward_scales.raibert_heuristic = 0.0 #-10.0
    # Cfg.reward_scales.feet_impact_vel = 0.0 #-0.0
    # Cfg.reward_scales.feet_clearance = 0.0 #-0.0
    # Cfg.reward_scales.feet_clearance_cmd = 0.0 #-0.0
    ...
    # Cfg.reward_scales.tracking_stance_width = 0.0 #-0.0
    # Cfg.reward_scales.tracking_stance_length = 0.0 #-0.0
    # Cfg.reward_scales.lin_vel_z = 0.0 #-0.02
    Cfg.reward_scales.ang_vel_xy = -0.001
    # Cfg.reward_scales.feet_air_time = 0.0 # 0.0
    # Cfg.reward_scales.hop_symmetry = 0.0 #0.0
    ...
    Cfg.reward_scales.collision = 0.0 #-5.0
    ...
    # TODO : commands for Nfg should be only position 
    Cfg.commands.lin_vel_x =  [-0.1,0.1] #[-1.0, 1.0]
    Cfg.commands.lin_vel_y =  [-0.1,0.1] #[-0.6, 0.6]
    Cfg.commands.ang_vel_yaw =  [-0.1,0.1] #[-1.0, 1.0]
    # Cfg.commands.body_height_cmd = [-0.25, 0.15]
    # TODO : irrespectively should the gait commands come into pic 
    ...
    Cfg.commands.limit_vel_x = [-0.5,0.5] #[-5.0, 5.0]
    Cfg.commands.limit_vel_y = [-0.5,0.5] #[-0.6, 0.6]
    Cfg.commands.limit_vel_yaw = [-0.5,0.5] #[-5.0, 5.0]
    ...
    # TODO : check the below cause gaitwise_curricula is passing through gait 
    # necessary for positional rewards
    Cfg.commands.binary_phases = True
    Cfg.commands.gaitwise_curricula = True

    # step 1 : create the envs : replicate the Velocity Tracking Env
    nav_env = NavigationTrackingEasyEnv(sim_device='cuda:0' , nfg = Nfg)    
    # step 2 : load the policies
    from ml_logger import logger
    ...
    num_eval_steps = 2000 # TODO : check is we need the evaluatoin steps
    ...
    # step 3 : get the observations from the loaded policy 
    # Step 4 : use History wrapper on them 
    # step 5 : Runner
    # Step 6 : runner.learn
...
if __name__ == '__main__':
    ...
    logger.configure(logger.utcnow(f'navigation-policy/%Y-%m-%d/{stem}/%H%M%S.%f'),
