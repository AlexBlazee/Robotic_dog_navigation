...
class LeggedRobot(BaseTask):
    ...
    def _teleport_robots(self, env_ids, cfg):
        ...
        if cfg.terrain.teleport_robots:
            ...
            self.gym.set_actor_root_state_tensor(self.sim, gymtorch.unwrap_tensor(self._merge_anymal_root_state_with_wall_root_state()))
            self.gym.refresh_actor_root_state_tensor(self.sim)
    ...
    # def _merge_anymal_dof_state_with_wall_dof_state(self):
    #     res = []
    #     for i,t in enumerate(self.dof_state):
    #         res.append(t)
    #         itr_index = i * 4
    #         for j in range(itr_index , itr_index + 4 ): 
    #             res.append(self.wall_dof_state[j])
    #     resTorch = torch.empty([len(res) , res[0].size()[0]] , device = self.device)
    #     return torch.stack(res , out = resTorch)
    ...
    def _add_boundary_walls_to_env(self , env_handle , i , origins) :
        ...
        self.actor_handles.append(box_handle1)
    ...
    def _get_env_origins(self, env_ids, cfg):
        ...
        if cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            if cfg.terrain.center_robots:
                ...
                # this makes sure that the terrain env are not randomized
                # print(" _get_env_origins Terrain levels preventing randomization ")
                # self.terrain_levels[env_ids] = torch.randint(min_terrain_level, min_terrain_level + 1, (len(env_ids),),
                #                                              device=self.device)
                # self.terrain_types[env_ids] = torch.randint(min_terrain_type, min_terrain_type + 1, (len(env_ids),),
                #                                             device=self.device)

                # print( "_get_env_origins Terrain levels : ",self.terrain_levels[env_ids])
                # print( "_get_env_origins Terrain types : ", self.terrain_types[env_ids])
