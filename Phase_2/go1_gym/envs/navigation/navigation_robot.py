...
from go1_gym.envs.navigation.base_task import BaseTask
...
class LeggedRobot(BaseTask):
    def __init__(self, nfg: Nfg, sim_params, physics_engine, sim_device, headless, eval_nfg=None,
        ...
        Args:
            nfg (Dict): Environment config file
        ...
        """
        self.nfg = nfg
        self.eval_nfg = eval_nfg
        # self.sim_params = sim_params
        # self.height_samples = None
        self.debug_viz = False
        self.init_done = False
        self.initial_dynamics_dict = initial_dynamics_dict
        if eval_nfg is not None: self._parse_nfg(eval_nfg)
        self._parse_nfg(self.nfg)

        super().__init__(self.nfg, sim_params, physics_engine, sim_device, headless, self.eval_nfg)
        ...
        if not self.headless:
            self.set_camera(self.nfg.viewer.pos, self.nfg.viewer.lookat)
        self._init_buffers()
    ...
    def step(self, actions):
        ...
        """
        clip_actions = self.nfg.normalization.clip_actions
        ...
        self.render_gui()
        for _ in range(self.nfg.control.decimation):
        ...
        clip_obs = self.nfg.normalization.clip_observations
    ...
    def check_termination(self):
        ...
        self.reset_buf = torch.any(torch.norm(self.contact_forces[:, self.termination_contact_indices, :], dim=-1) > 1.,
                                   dim=1)
        self.time_out_buf = self.episode_length_buf > self.nfg.env.max_episode_length  # no terminal reward for time-outs
        self.reset_buf |= self.time_out_buf
        if self.nfg.rewards.use_terminal_body_height:
            self.body_height_buf = torch.mean(self.root_states[:, 2].unsqueeze(1) - self.measured_heights, dim=1) \
                                   < self.nfg.rewards.terminal_body_height
    ...
    def reset_idx(self, env_ids):
        ...
        if self.nfg.domain_rand.randomize_rigids_after_start:
        ...
        if self.nfg.commands.command_curriculum:
            ...
            if self.nfg.commands.num_commands > 9:
    ...
    def compute_reward(self):
        ...
        if self.nfg.rewards.only_positive_rewards:
            self.rew_buf[:] = torch.clip(self.rew_buf[:], min=0.)
        elif self.nfg.rewards.only_positive_rewards_ji22_style: #TODO: update
            self.rew_buf[:] = self.rew_buf_pos[:] * torch.exp(self.rew_buf_neg[:] / self.nfg.rewards.sigma_rew_neg)
    ...
    def compute_observations(self):
        ...
        # if self.nfg.env.observe_command and not self.nfg.env.observe_height_command:
        #     self.obs_buf = torch.cat((self.projected_gravity,
        ...
        if self.nfg.env.observe_command:
            self.obs_buf = torch.cat((self.projected_gravity,
        ...
        if self.nfg.env.observe_two_prev_actions:
            self.obs_buf = torch.cat((self.obs_buf,
                                      self.last_actions), dim=-1)

        if self.nfg.env.observe_timing_parameter:
            self.obs_buf = torch.cat((self.obs_buf,
                                      self.gait_indices.unsqueeze(1)), dim=-1)

        if self.nfg.env.observe_clock_inputs:
            self.obs_buf = torch.cat((self.obs_buf,
                                      self.clock_inputs), dim=-1)

        # if self.nfg.env.observe_desired_contact_states:
        #     self.obs_buf = torch.cat((self.obs_buf,
        #                               self.desired_contact_states), dim=-1)

        if self.nfg.env.observe_vel:
            if self.nfg.commands.global_reference:
        ...
        if self.nfg.env.observe_only_ang_vel:
            self.obs_buf = torch.cat((self.base_ang_vel * self.obs_scales.ang_vel,
                                      self.obs_buf), dim=-1)

        if self.nfg.env.observe_only_lin_vel:
            self.obs_buf = torch.cat((self.base_lin_vel * self.obs_scales.lin_vel,
                                      self.obs_buf), dim=-1)

        if self.nfg.env.observe_yaw:
        ...
        if self.nfg.env.observe_contact_states:
        ...
        if self.nfg.env.priv_observe_com_displacement:
            com_displacements_scale, com_displacements_shift = get_scale_shift(
                self.nfg.normalization.com_displacement_range)
        ...
        if self.nfg.env.priv_observe_body_velocity:
            body_velocity_scale, body_velocity_shift = get_scale_shift(self.nfg.normalization.body_velocity_range)
            self.privileged_obs_buf = torch.cat((self.privileged_obs_buf,
            ...
            self.next_privileged_obs_buf = torch.cat((self.next_privileged_obs_buf,
                                                      ((self.base_lin_vel).view(self.num_envs,
                                                                                -1) - body_velocity_shift) * body_velocity_scale),
                                                     dim=1)
        if self.nfg.env.priv_observe_clock_inputs:
            self.privileged_obs_buf = torch.cat((self.privileged_obs_buf,
                                                 self.clock_inputs), dim=-1)

        assert self.privileged_obs_buf.shape[
                   1] == self.nfg.env.num_privileged_obs, f"num_privileged_obs ({self.nfg.env.num_privileged_obs}) != the number of privileged observations ({self.privileged_obs_buf.shape[1]}), you will discard data from the student!"
    ...
    #     mesh_type = self.nfg.terrain.mesh_type
    #     if mesh_type in ['heightfield', 'trimesh']:
    #         if self.eval_nfg is not None:
    #             self.terrain = Terrain(self.nfg.terrain, self.num_train_envs, self.eval_nfg.terrain, self.num_eval_envs)
    #         else:
    #             self.terrain = Terrain(self.nfg.terrain, self.num_train_envs)
    #     if mesh_type == 'plane':
    ...
    def _call_train_eval(self, func, env_ids):
        ...
        if len(env_ids_train) > 0:
            ret = func(env_ids_train, self.nfg)
        if len(env_ids_eval) > 0:
            ret_eval = func(env_ids_eval, self.eval_nfg)
    ...
    #     elif self.nfg.domain_rand.randomize_gravity:
    #         min_gravity, max_gravity = self.nfg.domain_rand.gravity_range
    ...
    def _process_dof_props(self, props, env_id):
        ...
        if env_id == 0:
            ...
            for i in range(len(props)):
                ...
                self.dof_pos_limits[i, 0] = m - 0.5 * r * self.nfg.rewards.soft_dof_pos_limit
                self.dof_pos_limits[i, 1] = m + 0.5 * r * self.nfg.rewards.soft_dof_pos_limit

        return props

    def _randomize_rigid_body_props(self, env_ids, nfg):
        if nfg.domain_rand.randomize_base_mass:
            min_payload, max_payload = nfg.domain_rand.added_mass_range
        ...
        if nfg.domain_rand.randomize_com_displacement:
            min_com_displacement, max_com_displacement = nfg.domain_rand.com_displacement_range
    ...
    # def refresh_actor_rigid_shape_props(self, env_ids, nfg):
    ...
    def _randomize_dof_props(self, env_ids, nfg):
        if nfg.domain_rand.randomize_motor_strength:
            min_strength, max_strength = nfg.domain_rand.motor_strength_range
            self.motor_strengths[env_ids, :] = torch.rand(len(env_ids), dtype=torch.float, device=self.device,
                                                     requires_grad=False).unsqueeze(1) * (
                                                  max_strength - min_strength) + min_strength
        if nfg.domain_rand.randomize_motor_offset:
            min_offset, max_offset = nfg.domain_rand.motor_offset_range
            self.motor_offsets[env_ids, :] = torch.rand(len(env_ids), self.num_dof, dtype=torch.float,
                                                        device=self.device, requires_grad=False) * (
                                                     max_offset - min_offset) + min_offset
        if nfg.domain_rand.randomize_Kp_factor:
            min_Kp_factor, max_Kp_factor = nfg.domain_rand.Kp_factor_range
        ...
        if nfg.domain_rand.randomize_Kd_factor:
            min_Kd_factor, max_Kd_factor = nfg.domain_rand.Kd_factor_range
    ...
    def _post_physics_step_callback(self):
        ...
        # resample commands
        sample_interval = int(self.nfg.commands.resampling_time / self.dt)
        env_ids = (self.episode_length_buf % sample_interval == 0).nonzero(as_tuple=False).flatten()
        self._resample_commands(env_ids)
        self._step_contact_targets()

        # measure terrain heights
        if self.nfg.terrain.measure_heights:
            self.measured_heights = self._get_heights(torch.arange(self.num_envs, device=self.device), self.nfg)

        # push robots
        self._call_train_eval(self._push_robots, torch.arange(self.num_envs, device=self.device))

        # randomize dof properties
        env_ids = (self.episode_length_buf % int(self.nfg.domain_rand.rand_interval) == 0).nonzero(
            as_tuple=False).flatten()
        self._call_train_eval(self._randomize_dof_props, env_ids)

        if self.common_step_counter % int(self.nfg.domain_rand.gravity_rand_interval) == 0:
            self._randomize_gravity()
        if int(self.common_step_counter - self.nfg.domain_rand.gravity_rand_duration) % int(
                self.nfg.domain_rand.gravity_rand_interval) == 0:
    ...
    def _resample_commands(self, env_ids):
        ...
        timesteps = int(self.nfg.commands.resampling_time / self.dt)
        ep_len = min(self.nfg.env.max_episode_length, timesteps)
        ...
        for i, (category, env_ids_in_category, curriculum) in enumerate(
            ...
            self.commands[env_ids_in_category, :] = torch.Tensor(new_commands[:, :self.nfg.commands.num_commands]).to(
                self.device)

        if self.nfg.commands.num_commands > 5:
            if self.nfg.commands.gaitwise_curricula:
            ...
            elif self.nfg.commands.exclusive_phase_offset:
                random_env_floats = torch.rand(len(env_ids), device=self.device)
            ...
            elif self.nfg.commands.balance_gait_distribution:
                random_env_floats = torch.rand(len(env_ids), device=self.device)
            ...
            if self.nfg.commands.binary_phases:
    ...
    def _step_contact_targets(self):
        if self.nfg.env.observe_gait_commands:
            ...
            if self.nfg.commands.pacing_offset:
            ...
            # if self.nfg.commands.durations_warp_clock_inputs:
            ...
            kappa = self.nfg.rewards.kappa_gait_probs
    ...
    def _compute_torques(self, actions):
        ...
        """
        # pd controller
        actions_scaled = actions[:, :12] * self.nfg.control.action_scale
        actions_scaled[:, [0, 3, 6, 9]] *= self.nfg.control.hip_scale_reduction  # scale down hip flexion range

        if self.nfg.domain_rand.randomize_lag_timesteps:
            self.lag_buffer = self.lag_buffer[1:] + [actions_scaled.clone()]
            self.joint_pos_target = self.lag_buffer[0] + self.default_dof_pos
        else:
            self.joint_pos_target = actions_scaled + self.default_dof_pos

        control_type = self.nfg.control.control_type
    ...
    def _reset_dofs(self, env_ids, nfg):
    ...
    def _reset_root_states(self, env_ids, nfg):
        ...
        if self.custom_origins:
            ...
            self.root_states[env_ids, 0:1] += torch_rand_float(-nfg.terrain.x_init_range,
                                                               nfg.terrain.x_init_range, (len(env_ids), 1),
                                                               device=self.device)
            self.root_states[env_ids, 1:2] += torch_rand_float(-nfg.terrain.y_init_range,
                                                               nfg.terrain.y_init_range, (len(env_ids), 1),
                                                               device=self.device)
            self.root_states[env_ids, 0] += nfg.terrain.x_init_offset
            self.root_states[env_ids, 1] += nfg.terrain.y_init_offset
        else:
            self.root_states[env_ids] = self.base_init_state
            self.root_states[env_ids, :3] += self.env_origins[env_ids]

        # base yaws
        init_yaws = torch_rand_float(-nfg.terrain.yaw_init_range,
                                     nfg.terrain.yaw_init_range, (len(env_ids), 1),
                                     device=self.device)
        ...
        if nfg.env.record_video and 0 in env_ids:
        ...
        if nfg.env.record_video and self.eval_nfg is not None and self.num_train_envs in env_ids:
    ...
    def _get_noise_scale_vec(self, nfg):
        ...
        self.add_noise = self.nfg.noise.add_noise
        noise_scales = self.nfg.noise_scales
        noise_level = self.nfg.noise.noise_level
        ...
        if self.nfg.env.observe_command:
            noise_vec = torch.cat((torch.ones(3) * noise_scales.gravity * noise_level,
                                   torch.zeros(self.nfg.commands.num_commands),
                                   torch.ones(
                                       self.num_actuated_dof) * noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos,
                                   torch.ones(
                                       self.num_actuated_dof) * noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel,
    ...
    def _init_buffers(self):
        ...
        self.lag_buffer = [torch.zeros_like(self.dof_pos) for i in range(self.nfg.domain_rand.lag_timesteps+1)]
        ...
        # if self.nfg.terrain.measure_heights:
        #     self.height_points = self._init_height_points(torch.arange(self.num_envs, device=self.device), self.nfg)
        # self.measured_heights = 0

        self.noise_scale_vec = self._get_noise_scale_vec(self.nfg)  # , self.eval_nfg)
        ...
        self.commands_value = torch.zeros(self.num_envs, self.nfg.commands.num_commands, dtype=torch.float,
                                          device=self.device, requires_grad=False)
        ...
        self.commands_scale = torch.tensor([self.obs_scales.lin_vel, self.obs_scales.lin_vel, self.obs_scales.ang_vel,
                                           ...
                                           device=self.device, requires_grad=False, )[:self.nfg.commands.num_commands]
        ...
        for i in range(self.num_dofs):
            ...
            angle = self.nfg.init_state.default_joint_angles[name]
            self.default_dof_pos[i] = angle
            found = False
            for dof_name in self.nfg.control.stiffness.keys():
                if dof_name in name:
                    self.p_gains[i] = self.nfg.control.stiffness[dof_name]
                    self.d_gains[i] = self.nfg.control.damping[dof_name]
                    found = True
            if not found:
                self.p_gains[i] = 0.
                self.d_gains[i] = 0.
                if self.nfg.control.control_type in ["P", "V"]:
                    print(f"PD gain of joint {name} were not defined, setting them to zero")
        self.default_dof_pos = self.default_dof_pos.unsqueeze(0)

        if self.nfg.control.control_type == "actuator_net":
    ...
    def _init_command_distribution(self, env_ids):
        ...
        if self.nfg.commands.curriculum_type == "RewardThresholdCurriculum":
            from .curriculum import RewardThresholdCurriculum
            CurriculumClass = RewardThresholdCurriculum
        self.curricula = []
        for category in self.category_names:
            self.curricula += [CurriculumClass(seed=self.nfg.commands.curriculum_seed,
                                               x_vel=(self.nfg.commands.limit_vel_x[0],
                                                      self.nfg.commands.limit_vel_x[1],
                                                      self.nfg.commands.num_bins_vel_x),
                                               y_vel=(self.nfg.commands.limit_vel_y[0],
                                                      self.nfg.commands.limit_vel_y[1],
                                                      self.nfg.commands.num_bins_vel_y),
                                               yaw_vel=(self.nfg.commands.limit_vel_yaw[0],
                                                        self.nfg.commands.limit_vel_yaw[1],
                                                        self.nfg.commands.num_bins_vel_yaw),
                                               body_height=(self.nfg.commands.limit_body_height[0],
                                                            self.nfg.commands.limit_body_height[1],
                                                            self.nfg.commands.num_bins_body_height),
                                               gait_frequency=(self.nfg.commands.limit_gait_frequency[0],
                                                               self.nfg.commands.limit_gait_frequency[1],
                                                               self.nfg.commands.num_bins_gait_frequency),
                                               gait_phase=(self.nfg.commands.limit_gait_phase[0],
                                                           self.nfg.commands.limit_gait_phase[1],
                                                           self.nfg.commands.num_bins_gait_phase),
                                               gait_offset=(self.nfg.commands.limit_gait_offset[0],
                                                            self.nfg.commands.limit_gait_offset[1],
                                                            self.nfg.commands.num_bins_gait_offset),
                                               gait_bounds=(self.nfg.commands.limit_gait_bound[0],
                                                            self.nfg.commands.limit_gait_bound[1],
                                                            self.nfg.commands.num_bins_gait_bound),
                                               gait_duration=(self.nfg.commands.limit_gait_duration[0],
                                                              self.nfg.commands.limit_gait_duration[1],
                                                              self.nfg.commands.num_bins_gait_duration),
                                               footswing_height=(self.nfg.commands.limit_footswing_height[0],
                                                                 self.nfg.commands.limit_footswing_height[1],
                                                                 self.nfg.commands.num_bins_footswing_height),
                                               body_pitch=(self.nfg.commands.limit_body_pitch[0],
                                                           self.nfg.commands.limit_body_pitch[1],
                                                           self.nfg.commands.num_bins_body_pitch),
                                               body_roll=(self.nfg.commands.limit_body_roll[0],
                                                          self.nfg.commands.limit_body_roll[1],
                                                          self.nfg.commands.num_bins_body_roll),
                                               stance_width=(self.nfg.commands.limit_stance_width[0],
                                                             self.nfg.commands.limit_stance_width[1],
                                                             self.nfg.commands.num_bins_stance_width),
                                               stance_length=(self.nfg.commands.limit_stance_length[0],
                                                                self.nfg.commands.limit_stance_length[1],
                                                                self.nfg.commands.num_bins_stance_length),
                                               aux_reward_coef=(self.nfg.commands.limit_aux_reward_coef[0],
                                                           self.nfg.commands.limit_aux_reward_coef[1],
                                                           self.nfg.commands.num_bins_aux_reward_coef),
                                               )]

        if self.nfg.commands.curriculum_type == "LipschitzCurriculum":
            for curriculum in self.curricula:
                curriculum.set_params(lipschitz_threshold=self.nfg.commands.lipschitz_threshold,
                                      binary_phases=self.nfg.commands.binary_phases)
        self.env_command_bins = np.zeros(len(env_ids), dtype=np.int)
        self.env_command_categories = np.zeros(len(env_ids), dtype=np.int)
        low = np.array(
            [self.nfg.commands.lin_vel_x[0], self.nfg.commands.lin_vel_y[0],
             self.nfg.commands.ang_vel_yaw[0], self.nfg.commands.body_height_cmd[0],
             self.nfg.commands.gait_frequency_cmd_range[0],
             self.nfg.commands.gait_phase_cmd_range[0], self.nfg.commands.gait_offset_cmd_range[0],
             self.nfg.commands.gait_bound_cmd_range[0], self.nfg.commands.gait_duration_cmd_range[0],
             self.nfg.commands.footswing_height_range[0], self.nfg.commands.body_pitch_range[0],
             self.nfg.commands.body_roll_range[0],self.nfg.commands.stance_width_range[0],
             self.nfg.commands.stance_length_range[0], self.nfg.commands.aux_reward_coef_range[0], ])
        high = np.array(
            [self.nfg.commands.lin_vel_x[1], self.nfg.commands.lin_vel_y[1],
             self.nfg.commands.ang_vel_yaw[1], self.nfg.commands.body_height_cmd[1],
             self.nfg.commands.gait_frequency_cmd_range[1],
             self.nfg.commands.gait_phase_cmd_range[1], self.nfg.commands.gait_offset_cmd_range[1],
             self.nfg.commands.gait_bound_cmd_range[1], self.nfg.commands.gait_duration_cmd_range[1],
             self.nfg.commands.footswing_height_range[1], self.nfg.commands.body_pitch_range[1],
             self.nfg.commands.body_roll_range[1],self.nfg.commands.stance_width_range[1],
             self.nfg.commands.stance_length_range[1], self.nfg.commands.aux_reward_coef_range[1], ])
        for curriculum in self.curricula:
            curriculum.set_to(low=low, high=high)

    def _prepare_reward_function(self):
        """ Prepares a list of reward functions, whcih will be called to compute the total reward.
            Looks for self._reward_<REWARD_NAME>, where <REWARD_NAME> are names of all non zero reward scales in the nfg.
        """
        # reward containers
        from go1_gym.envs.rewards.corl_rewards import CoRLRewards
        reward_containers = {"CoRLRewards": CoRLRewards}
        self.reward_container = reward_containers[self.nfg.rewards.reward_container_name](self)
    ...
    def _parse_nfg(self, nfg):
        self.dt = self.nfg.control.decimation * self.sim_params.dt
        self.obs_scales = self.nfg.obs_scales
        self.reward_scales = vars(self.nfg.reward_scales)
        # self.curriculum_thresholds = vars(self.nfg.curriculum_thresholds)
        nfg.command_ranges = vars(nfg.commands)
        # if nfg.terrain.mesh_type not in ['heightfield', 'trimesh']:
        #     nfg.terrain.curriculum = False
        max_episode_length_s = nfg.env.episode_length_s
        ...
        #     nfg.domain_rand.gravity_rand_interval * nfg.domain_rand.gravity_impulse_duration)
