...
class NavigationRobot:
    def __init__(self, cfg: Cfg, sim_params, physics_engine, sim_device, headless, eval_cfg=None,
        ...
        #basetask.py
        # below is the code that takes in important info from the basetask.py
        ...
        # print("NUMS OBSERVATION :", self.num_obs , self.num_privileged_obs , self.num_actions , self.num_train_envs , self.num_envs)
        ...
        # basetask.py implementation ends here

        # you have all the envs 
        ...
        self._init_buffers() #TODO : continue here
    ...
    def step(self, actions): # TODO : The actions should be lin_vel and angular_vel
        ...
        # print("clipped actions:" , self.actions)
        # step physics and render each frame
        self.prev_base_pos = self.base_pos.clone() # not used 
        self.prev_base_quat = self.base_quat.clone() # not used
        # self.prev_base_lin_vel = self.base_lin_vel.clone() # not used
        # self.prev_foot_velocities = self.foot_velocities.clone()
        # self.render_gui()
        #TODO: some thing like
    ...
    def post_physics_step(self):
        ...
        # TODO : Post physics step call above does this 
        # self.locomotion_env.post_physics_step_record_update(self.record_now)
        ...
        # print(" THE BASE POSITION OF ROBOT", self.base_pos )
        self.compute_reward()
    ...
    def check_termination(self):
        ...
        #                            dim=1)
        # TODO : below statement removed as it already is done in locomotion step call
        # self.locomotion_env.check_termination()
        
        self.reset_buf = self.locomotion_env.reset_buf.clone()        
        # print("RESET TIME LIMIT : ",self.cfg.env.max_episode_length )
    ...
    def reset_idx(self, env_ids):
        ...
        #     self.extras["train/episode"]["min_command_x_pos"] = torch.min(self.commands[:, 0])
        #     self.extras["train/episode"]["max_command_x_pos"] = torch.max(self.commands[:, 0])
        #     self.extras["train/episode"]["min_command_y_pos"] = torch.min(self.commands[:, 1])
        #     self.extras["train/episode"]["max_command_y_pos"] = torch.max(self.commands[:, 1])
    ...
    #     # base position
    #     self.locomotion_env/set_idx_pose(env_ids , dof)
    ...
    def compute_reward(self):
        ...
        for i in range(len(self.reward_functions)):
            ...
            # print("REWARD SCALES ", self.reward_scales)
            # print("REWARD FUNCTION ", self.reward_functions[i]())
        ...
        # print("ENV ORIGINS : " , self.locomotion_env.env_origins[:,0])
        # self.command_sums["lin_pos_raw"] += self.base_pos[:, 0] # taking only x value

        # self.command_sums["lin_pos_residual"] += (self.base_pos[:, 0] - self.commands[:,0]) ** 2
        # self.command_sums["ep_timesteps"] += 1
    ...
    def compute_observations(self):
        ...
        #                               self.actions
        #                               ), dim=-1)
        # print("OBSERVE COMMANDS")
        # print("self.command:", self.commands ,"* self.commands_scale ",self.commands_scale, ":::", self.commands * self.commands_scale )
        # print("self.base_pos:", self.base_pos ,"* self.obs_scales.lin_pos ",self.obs_scales.lin_pos, ":::", self.base_pos * self.obs_scales.lin_pos )
        # print("actions : " , self.actions)
        # print("last_action : ", self.last_actions)
        ...
        # if self.cfg.env.observe_contact_states: #used for termination on contact
        ...
        if self.add_noise:
            ...
            # print("noise", (2 * torch.rand_like(self.obs_buf) - 1) * self.noise_scale_vec)
        # build privileged obs

        self.privileged_obs_buf = torch.empty(self.num_envs, 0).to(self.device)  # has to be empty 
        self.next_privileged_obs_buf = torch.empty(self.num_envs, 0).to(self.device) # has to be empty
    ...
    """ _randomize_gravity , _process_rigid_shape_props , _process_dof_props , _randomize_rigid_body_props , refresh_actor_rigid_shape_props , 
    ...
    #     # self._resample_commands(env_ids)
    #     # self._step_contact_targets() - not necessary
    ...
    #         # print("COMMAND SUM ", self.command_sums)
    #         # print("env_ids_in_category", env_ids_in_category)
    #         # print("ep_len", ep_len)
    #         task_rewards, success_thresholds = [], []
    #         for key in ["tracking_lin_pos"]:
    ...
    #                 # print("TASK REWARD", task_rewards)
    #                 # print("SUCCESS THRESHOLD" , success_thresholds)

    #         old_bins = self.env_command_bins[env_ids_in_category.cpu().numpy()]
    #         # print("self.env_command_bins" , self.env_command_bins)
    #         if len(success_thresholds) > 0:
    #             # print("OLD BIINS:", old_bins)
    #             # print("task_rewards", task_rewards)
    #             # print("success_thresholds ", success_thresholds)
    ...
    #     # print("COMMANDS BEFORE NORM", self.commands)
    #     self.commands[env_ids, :2] *= (torch.norm(self.commands[env_ids, :2], dim=1) > 0.2).unsqueeze(1)
    #     # print("COMMANDS BEFORE ADDITION", self.commands)
    #     self.commands[env_ids , :2] += self.locomotion_env.env_origins[env_ids , :2] # not adding base init pos as its 0,0,0 in our case
    #     # reset command sums
    ...
    def _reset_root_states(self, env_ids, cfg):
        # TODO : just call the legged robot.py function here
        ...
        # self.video_frames_eval = self.locomotion_env.video_frame_eval
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec = torch.cat((
                            #    torch.zeros(self.cfg.commands.num_commands),
                                ...
                                # torch.zeros(4), # quat info # TODO : Not sure if I should add or not # TODO : how do you add noise to quat
                                # TODO : clarification is noise can't be given to quat but to yaw
                                torch.zeros(self.num_actions),
                                ), dim=0)
        ...
        #                            noise_vec
        #                            ), dim=0)
        # TODO : should you add this noise to yaw
        if self.cfg.env.observe_yaw:
            noise_vec = torch.cat((noise_vec,
                                   torch.zeros(1),
                                   ), dim=0)
    ...
    def _init_buffers(self):
        # TODO : 1. check the noise vector scales 2. should you add smootness reward 
        ...
        self.root_states = self.locomotion_env.root_states.clone()
        # print("ROOOT SATATES INIT BUF", self.root_states)
        self.base_pos = self.root_states[:self.num_envs, 0:3]
        ...
        self.prev_base_quat = self.base_quat.clone() # ADDED NEW
        # self.prev_foot_velocities = self.foot_velocities.clone()
        ...
        # TODO : check if the noise : check in detailed way
        ...
        # TODO : should be considered as if you want to add smoothnes
        # below code is used for action smoothenedss in rewards
        ...
        # self.last_root_vel = torch.zeros_like(self.root_states[:, 7:13]) #not used any where
        # self.last_root_pos = torch.zeros_like(self.root_states[: , 0:3]) # created new
        ...
        #     # TODO change below line to position lin_pos and
        #     self.obs_scales.lin_pos, self.obs_scales.lin_pos
        ...
        #                                      device=self.device,
        #                                      requires_grad=False)
        # self.base_lin_vel = quat_rotate_inverse(self.base_quat, self.root_states[:self.num_envs, 7:10]) # being sent to extras
        # self.base_ang_vel = quat_rotate_inverse(self.base_quat, self.root_states[:self.num_envs, 10:13]) # being sent to extras
        # self.projected_gravity = quat_rotate_inverse(self.base_quat, self.gravity_vec)
    ...
    #         # TODO : chnage these to x_pos and y_pos  #TODO : DONE
    #                                            x_pos=(self.cfg.commands.limit_pos_x[0],
    #                                                   self.cfg.commands.limit_pos_x[1],
    #                                                   self.cfg.commands.num_bins_pos_x),
    #                                            y_pos=(self.cfg.commands.limit_pos_y[0],
    #                                                   self.cfg.commands.limit_pos_y[1],
    #                                                   self.cfg.commands.num_bins_pos_y),
    ...
    #     # TODO : change them to lin_pos_x and lin_pos_y
    #     low = np.array(
    #         [ self.cfg.commands.lin_pos_x[0], self.cfg.commands.lin_pos_y[0],
    ...
    #         ])
    #     high = np.array(
    #         [self.cfg.commands.lin_pos_x[1], self.cfg.commands.lin_pos_y[1],            
    ...
    def _prepare_reward_function(self): #TODO : add reward function for location # TODO : added to walk these ways corl_rewards.py 
        """ Prepares a list of reward functions, whcih will be called to compute the total reward.
            Looks for self._reward_<REWARD_NAME>, where <REWARD_NAME> are names of all non zero reward scales in the cfg.
        """
        # reward containers
        # TODO : add a fucntion to reward position tracking - give other scales as 0
        # chnage the configuration for this
        ...
        #     name: torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        #     for name in
        #     # TODO : add lin_pos_raw , lin_pos_residual, lin_yaw_ **** # TODO done
        #     list(self.reward_scales.keys()) + ["lin_pos_raw", "lin_pos_residual",  "ep_timesteps"]}
    ...
    def _create_envs(self):
        ...
        # print("create env self.locomotion_env.obs", self.locomotion_env.obs)
        # num_eval_steps = 50
        # gaits = {"pronking": [0, 0, 0],
        #         "trotting": [0.5, 0, 0],
        #         "bounding": [0, 0.5, 0],
        ...
        # obs = self.locomotion_env.reset()

        # frames = []
        
        # for i in tqdm(range(num_eval_steps)):
        #     with torch.no_grad():
        #         actions = self.locomotion_policy(obs)
        #     self.locomotion_env.commands[:, 0] = x_vel_cmd_array[i]
        #     self.locomotion_env.commands[:, 1] = y_vel_cmd_array[i]
        #     self.locomotion_env.commands[:, 2] = yaw_vel_cmd_array[i]
        #     self.locomotion_env.commands[:, 3] = body_height_cmd
        #     self.locomotion_env.commands[:, 4] = step_frequency_cmd
        #     self.locomotion_env.commands[:, 5:8] = gait
        #     self.locomotion_env.commands[:, 8] = 0.5
        ...
        #     self.locomotion_env.commands[:, 11] = roll_cmd
        #     self.locomotion_env.commands[:, 12] = stance_width_cmd
        #     obs, rew, done, info = self.locomotion_env.step(actions)
        #     # print("Observational bf", obs )
        #     # print("REW buf" , rew)
        #     # print("DONE buf", done)
        #     # print("INFO buf", info)


        #     measured_x_vels[i] = self.locomotion_env.base_lin_vel[0, 0]
        #     measured_y_vels[i] = self.locomotion_env.base_lin_vel[0 ,1]
        #     measured_yaw_vels[i] = self.locomotion_env.base_ang_vel[0, 2]

        #     joint_positions[i] = self.locomotion_env.dof_pos[0, :].cpu()
        #     frames.append(self.locomotion_env.render())

        # if len(frames) > 0:
        #     print("LOGGING VIDEO")
        #     logger.save_video(frames, "play_demo.mp4", fps=1 / self.locomotion_env.dt)
        # TODO : Below code was commented to meet the recording issues
        # self.locomotion_env.camera_setup(self.cfg.env.record_video)
    ...
    #     return self.locomotion_env.render(mode="rgb_array")
    ...
    #     self.locomotion_env.render_headless()
    #     self.video_frames = self.locomotion_env.video_frames
    #     # self.video_frames_eval = self.locomotion_env.video_frame_eval
    #     self.complete_video_frames = self.locomotion_env.complete_video_frames
    #     self.complete_video_frames_eval = self.locomotion_env.complete_video_frames_eval
    ...
    def get_complete_frames_eval(self):
        return self.locomotion_env.get_complete_frames_eval()
