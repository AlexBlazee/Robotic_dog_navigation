...
class NavigationRobot:
    def __init__(self, cfg: Cfg, sim_params, physics_engine, sim_device, headless, eval_cfg=None,
        ...
        """
        print("Entered Navigation Robot\n")
        self.cfg = cfg
    ...
    def step(self, actions): # TODO : The actions should be lin_vel and angular_vel
        ...
        with torch.no_grad():
            locomotion_actions = self.locomotion_policy(self.locomotion_env.obs_buf.clone())
        self.locomotion_env.commands[:, 0] = actions[:,0]
        self.locomotion_env.commands[:, 1] = actions[:,1]
        self.locomotion_env.commands[:, 2] = actions[:,2]
        ...
        locomotion_obs, locomotion_rew, locomotion_done, locomotion_info = self.locomotion_env.step(locomotion_actions)
    ...
    def compute_observations(self):
        ...
        if self.cfg.env.observe_command:
            self.obs_buf = torch.cat((
                                      ...
                                      self.base_pos * self.obs_scales.lin_pos,
                                      self.actions
                                      ), dim=-1)
        ...
        print("HEADING IUWBDIUEB DI:", heading)
    ...
    def _reset_root_states(self, env_ids, cfg):
        ...
        if not self.custom_origins:
            self.root_states[env_ids] = self.locomotion_env.root_states.clone()
    ...
    def load_env(self, label, headless=False):
        ...
        self.locomotion_Cfg.env.num_train_envs = 4
        self.locomotion_Cfg.env.num_eval_envs = 0
        self.locomotion_Cfg.env.num_envs = 4
        ...
        locomotion_env = HistoryWrapper(locomotion_env)

        # load policy
        from ml_logger import logger
        from go1_gym_learn.ppo_cse.actor_critic import ActorCritic

        locomotion_policy = self.load_policy(logdir)
    ...
    def _create_envs(self):
        ...
        self.load_env(label , headless=False )
        # num_eval_steps = 50
        # gaits = {"pronking": [0, 0, 0],
        #         "trotting": [0.5, 0, 0],
        #         "bounding": [0, 0.5, 0],
